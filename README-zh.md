...
SharePlan 桌面客户端是 SharePlan P2P AI 订阅共享平台的桌面应用。它连接到 SharePlan 云端服务器，在供应者和消费者之间中继任务，所有 LLM 调用都在本机通过 [cc-switch](https://github.com/farion1231/cc-switch) 执行。
...
**供应者** — 你的 cc-switch 代理接收来自云端的任务，用你的密钥调用上游 API，然后把结果返回。你按 token 数赚取积分。

**消费者** — 本地 OpenAI 兼容服务（`:8081`）将你的请求转发到云端，云端调度给最优供应者。你按 token 数消耗积分。

## 核心特性

- **供应者模式** — 共享闲置 AI 订阅时间赚取积分（90% 的 token 费用归你）
- **消费者模式** — 通过本地 OpenAI 兼容服务访问共享池中的任意模型
...
2. 点击 **启动本地服务** — 在 `127.0.0.1:8081` 启动
...
`share.db` SQLite 文件包含共享设置、钱包数据、任务日志和认证状态。删除此目录可重置所有数据。

## 技术栈

| 层 | 技术 |
|----|------|
| 后端 | Rust (Tauri v2)、SQLite (rusqlite)、tokio、reqwest、tokio-tungstenite |
...
│   ├── proxy_client.rs     支持流式的 HTTP 客户端
│   ├── proxy_executor.rs   通过 cc-switch 执行任务（Anthropic/OpenAI/Gemini 格式）
│   └── provider_registry.rs 从 cc-switch /status 端点发现 Provider
├── commands/          Tauri IPC 命令处理器
├── content_filter/    基于关键词的内容审核
├── credits/           按 Token 定价、结算、钱包
├── database/          独立 share.db（Schema v4、DAO）
├── local_server/      OpenAI 兼容 HTTP 服务器（:8081）消费者模式
├── share/             P2P 核心
│   ├── client.rs           带重连的 WebSocket 客户端
│   ├── daemon.rs           守护进程生命周期管理
│   ├── supplier.rs         任务执行（内容过滤 + 互斥检查）
│   ├── consumer.rs         云端调度 HTTP 客户端
│   ├── protocol.rs         消息类型（NodeStatus、TaskPayload、TaskResult）
│   ├── signing.rs           HMAC-SHA256 客户端签名
│   ├── fingerprint.rs       设备指纹生成
│   ├── web_bridge.rs       浏览器扩展本地 WS 服务器（:19829）
│   └── web_executor.rs     路由 web: Provider 任务到扩展
