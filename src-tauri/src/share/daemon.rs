...
use crate::share::protocol::{NodeState, NodeStatus, TaskPayload, TaskStatus};
...
impl Daemon {
    pub fn new(db: Arc<ShareDb>, executor: SharedExecutor, default_provider_id: String, provider_registry: Arc<ProviderRegistry>) -> Self {
        Self {
            db,
            executor,
            default_provider_id,
...
mod tests {
    ...
    fn test_daemon_new_idle() {
        let d = Daemon::new(create_test_db(), Arc::new(NullExecutor), "default".into(), create_test_registry());
        assert!(!d.is_running());
    ...
    async fn test_daemon_double_start_errors() {
        let mut d = Daemon::new(create_test_db(), Arc::new(NullExecutor), "default".into(), create_test_registry());
